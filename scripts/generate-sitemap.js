...
const siteURL = 'https://gerniile.pages.dev';
...
async function generateSitemap() {
	...
	for (const route of routes) {
		...
		if (route.dynamic) {
			const files = await fs.readdir(new URL(`../content${route.path}`, import.meta.url));

			for (const file of files) {
				content += formatPage(`${siteURL}${route.path}/${file.replace('.md', '')}`, publishDate);
			}
		}
	}
